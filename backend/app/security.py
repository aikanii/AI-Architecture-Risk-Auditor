...
def redact_secret(value: str) -> str:
    ...
    redacted = re.sub(r'sk-[a-zA-Z0-9]+', '[REDACTED_KEY]', redacted)  # OpenAI keys
...
def redact_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if any(secret_key in key.lower() for secret_key in ['secret', 'password', 'key', 'token', 'credential']):
            ...
            redacted[key] = [redact_dict(v) if isinstance(v, dict) else redact_secret(str(v)) if isinstance(v, str) else v for v in value]
